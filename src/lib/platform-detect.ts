...
const PLATFORMS: {
...
}[] = [
  {
    name: "Framer",
    detect: (html) =>
      /<meta[^>]*name=["']generator["'][^>]*content=["'][^"']*Framer/i.test(
        html,
      ) ||
    ...
    watermarks: [
      {
        selector: "framer-badge",
        description: "Framer badge overlay",
        pattern:
          /<div[^>]*class=["'][^"']*framer-badge[^"']*["'][^>]*>[\s\S]*?<\/div>/gi,
      },
      {
        selector: "framer-badge-link",
        description: "Made in Framer link",
        pattern:
          /<a[^>]*href=["'][^"']*framer\.com[^"']*["'][^>]*>[\s\S]*?Made\s+(?:in|with)\s+Framer[\s\S]*?<\/a>/gi,
      },
      {
        selector: "framer-overlay-script",
        description: "Framer overlay script",
        pattern:
          /<script[^>]*(?:framer-badge|framerBadge|framer-overlay)[^>]*>[\s\S]*?<\/script>/gi,
      },
      {
        selector: "framer-edit-button",
        description: "Edit in Framer button",
        pattern:
          /<div[^>]*id=["']framer-edit-button["'][^>]*>[\s\S]*?<\/div>/gi,
      },
      {
        selector: "framer-feedback",
        description: "Framer feedback bar",
        pattern:
          /<div[^>]*class=["'][^"']*framer-feedback[^"']*["'][^>]*>[\s\S]*?<\/div>/gi,
      },
      {
        selector: "framer-force-editor",
        description: "Framer editor bar script",
        pattern:
  ...
  },
  {
    ...
    watermarks: [
      {
        selector: "webflow-badge",
        description: "Webflow badge",
        pattern:
          /<a[^>]*class=["'][^"']*w-webflow-badge[^"']*["'][^>]*>[\s\S]*?<\/a>/gi,
      },
      {
        selector: "made-in-webflow",
        description: "Made in Webflow link",
        pattern:
          /<a[^>]*href=["'][^"']*webflow\.com[^"']*["'][^>]*>[\s\S]*?(?:Made\s+in|Powered\s+by)\s+Webflow[\s\S]*?<\/a>/gi,
      },
    ],
  },
  {
    ...
    watermarks: [
      {
        selector: "wix-ads",
        description: "Wix advertisement banner",
        pattern: /<div[^>]*id=["']WIX_ADS[^"']*["'][^>]*>[\s\S]*?<\/div>/gi,
      },
      {
        selector: "wix-badge",
        description: "Wix promotional badge",
        pattern:
          /<a[^>]*href=["'][^"']*wix\.com[^"']*["'][^>]*>[\s\S]*?(?:Create|Build|Made)\s+(?:a\s+)?(?:Free\s+)?(?:Website|with)\s*(?:Wix)?[\s\S]*?<\/a>/gi,
      },
    ],
  },
  {
    ...
    watermarks: [
      {
        selector: "squarespace-announcement",
        description: "Squarespace announcement bar",
        pattern:
          /<div[^>]*class=["'][^"']*squarespace-announcement-bar[^"']*["'][^>]*>[\s\S]*?<\/div>/gi,
      },
      {
        selector: "squarespace-badge",
        description: "Squarespace badge",
        pattern:
          /<a[^>]*href=["'][^"']*squarespace\.com[^"']*["'][^>]*>[\s\S]*?(?:Powered\s+by|Built\s+on)\s+Squarespace[\s\S]*?<\/a>/gi,
      },
    ],
  },
  {
    ...
    watermarks: [
      {
        selector: "wp-admin-bar",
        description: "WordPress admin bar",
        pattern: /<div[^>]*id=["']wpadminbar["'][^>]*>[\s\S]*?<\/div>/gi,
      },
    ],
  },
  {
    name: "Carrd",
    detect: (html) =>
      /<meta[^>]*name=["']generator["'][^>]*content=["'][^"']*Carrd/i.test(
        html,
      ) || /carrd\.co/i.test(html),
    watermarks: [
      {
        selector: "carrd-badge",
        description: "Carrd attribution badge",
        pattern:
          /<a[^>]*href=["'][^"']*carrd\.co[^"']*["'][^>]*>[\s\S]*?<\/a>/gi,
      },
    ],
  },
  {
    ...
    watermarks: [
      {
        selector: "notion-badge",
        description: "Built with Notion badge",
        pattern:
          /<a[^>]*href=["'][^"']*notion\.so[^"']*["'][^>]*>[\s\S]*?(?:Built|Made|Powered)\s+(?:with|by)\s+Notion[\s\S]*?<\/a>/gi,
      },
    ],
  },
  {
    ...
    watermarks: [
      {
        selector: "shopify-badge",
        description: "Powered by Shopify link",
        pattern:
          /<a[^>]*href=["'][^"']*shopify\.com[^"']*["'][^>]*>[\s\S]*?(?:Powered\s+by)\s+Shopify[\s\S]*?<\/a>/gi,
      },
    ],
  },
];
...
export function detectPlatform(html: string, url: string): PlatformResult {
  for (const platform of PLATFORMS) {
    if (platform.detect(html, url)) {
      const activeWatermarks = platform.watermarks.filter((w) =>
        w.pattern.test(html),
      );
...
export function stripWatermarks(html: string): {
  html: string;
  removed: number;
} {
  let result = html;
